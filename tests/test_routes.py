...
class TestProductRoutes(TestCase):
    ...
    def test_update_nonexistent_product(self):
        """It shouldn't update a nonexisting product"""
        # Call the update_products endpoint with a non-existent product ID
        response = self.client.put('/products/999', json={'name': 'New Product Name'})
        
        # Check if the response status code is 404
        self.assertEqual(response.status_code, 404)
        
        # Check if the response message indicates that the product was not found
        expected_message = "Product with id '999' was not found."
